...
local function findParms(s,tag,parms)
	...
	while true do
		...
		table.insert(parms, {_type = _type,_name = _name})
	end
	return parms
end
...
function getPF(s)
	...
	local resPF = [[
LCPB_]] .. pfName..[[ = class("LCPB_]] .. pfName .. [[",LCProtoBase)
function LCPB_]] .. pfName .. [[:ctor()
	local msg = {}
	self.super.ctor(self,protoPack,"]] .. pfName .. [[",msg)
	self.print = function()	
