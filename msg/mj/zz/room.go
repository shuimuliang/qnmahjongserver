...
func (r *MJRoom) SuanFen() {
	if r.RoomHupai.IsZiMoHupai {
		...
		banerPos := r.RoomStatus.BankerPos
		for _, seat := range r.RoomSeats {
			jiadi := int32(0)
			if r.RoomConfig.Config.Zhuangjiajiadi {
				if banerPos == ziMoPos {
					jiadi = 1
				}
				if banerPos == seat.Pos {
					jiadi = 1
				}
			}
			if seat.Pos != ziMoPos {
				paoScore := ziMoSeat.PaoScore + seat.PaoScore
				huScore := 1 + paoScore + jiadi
		...
		jiadi := int32(0)
		if r.RoomConfig.Config.Zhuangjiajiadi {
			if dianPaoPos == banerPos || JiePaoPos == banerPos {
				jiadi = 1
			}
		}
		huScore := 1 + paoScore + jiadi
...
func (r *MJRoom) AddAnGangScore(desPos int32) {
	...
	bankerPos := r.RoomStatus.BankerPos
	for _, seat := range r.RoomSeats {
		if seat.Pos != desPos {
			...
			if bankerPos != seat.Pos && bankerPos != desPos {
				jiadi = 0
			}
			gangSeat.GangScore += 1 + paoScore + jiadi
			seat.GangScore -= 1 + paoScore + jiadi
		}
	}
}
...
func (r *MJRoom) AddMingGangScore(srcPos int32, desPos int32) {
	...
	if bankerPos != srcPos && bankerPos != desPos {
		jiadi = 0
	}
	r.RoomSeats[srcPos].GangScore -= 1 + paoScore + jiadi
	r.RoomSeats[desPos].GangScore += 1 + paoScore + jiadi
}
