...
func (tiles Tiles) DelHunCard() (result Tiles) {
	for _, tile := range tiles {
		if tile.IsHun {
			continue
		}
...
func (tiles Tiles) CountHunCard(front int32) (hunCnt int32) {
	for i, tile := range tiles {
		if tile.Front == front {
			hunCnt++
			tiles[i].IsHun = true
...
func (tiles Tiles) CaluHunCnt(unicode int32) (hunCnt int32) {
	for _, tile := range tiles {
		if tile.Unicode == unicode {
			hunCnt++
		}
	}
	return
