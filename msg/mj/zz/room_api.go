...
func (m RoomMap) handleCreateRoom(
	...
	var roomID int32
	for {
		roomID = rand.Int31n(9000) + 1000 + 20000
...
func (m RoomMap) handleEnterRoom(
	...
	for pos, seat := range room.RoomSeats {
		if seat == nil {
			...
			cache.SetRoomID(id, roomID, def.MjTypeZz)
			success = true
			break
		}
	}
...
func (m RoomMap) handleGameRelink(
	...
	buyPaoListRecv *pf.BuyPaoListRecv,
	...
	if room.RoomStatus.GameStatus == def.GameStatusIsGaming {
		if room.RoomStatus.buyPaoListRecv != nil {
			buyPaoListRecv.Status = room.RoomStatus.buyPaoListRecv.GetStatus()
		}
...
func (m RoomMap) handlePrepareGame(
	...
	// 买跑不开局
	if room.RoomConfig.Config.Maipao {
		buyPaoListRecv.Status = def.StatusOK
		room.RoomStatus.buyPaoListRecv = buyPaoListRecv
		return
	}
	...
	*bankerID = bankerSeat.ID
	...
	// 断线重连数据
	if operationNoticeRecv.GetStatus() == def.StatusOK {
		room.RoomStatus.buyPaoListRecv = nil
	...
	RoomCache.mmap[roomID].PrintTile("开局后")
	return
}
...
func (m RoomMap) handleBuyPao(
	...
	if room.RoomSeats[pos].IsBuyPao {
		recv.Status = def.StatusBuyPao
		return
	}

	room.RoomSeats[pos].IsBuyPao = true
	room.RoomSeats[pos].PaoScore = send.PaoScore
	recv.PaoScore = send.PaoScore
	recv.Pos = pos + 1
	...
	if buyPaoCnt < def.RoomPlayerCount {
		return
	}
...
func (m RoomMap) handleDiscard(
	...
	// 碰杠
	if !room.RoomConfig.Config.Daihuner || room.RoomConfig.HunTile.Front != card {
		if len(operationNoticeRecv.OperationList) == 0 {
			for i := int32(1); i < def.RoomPlayerCount; i++ {
				operationPos := (discardPos + i) % def.RoomPlayerCount
				operationSeat := room.RoomSeats[operationPos]
...
func (m RoomMap) handleOperation(
	...
	case def.BuGang:
		...
		room.AddBuGangScore(srcPos, desPos)
	...
	case def.DianpaoHu:
		if room.RoomHupai.IsDianPao {
			...
			room.SuanFen()
			operation.CardsList = room.RoomHupai.DianPaoHuTiles.DelHuCard().GetFrontCards()
