...
func HandleEnterRoom(s *melody.Session, mjPlayer *cache.MjPlayer, send *pf.EnterRoomSend, abs *pf.AbsMessage) (err error) {
	...
	// 断线人发买跑消息
	if buyPaoListRecv.GetStatus() == def.StatusOK {
		msgBody, err := buyPaoListRecv.Marshal()
		if err == nil {
			absMessage := &pf.AbsMessage{
				MsgID:   int32(pf.BuyPaoList),
				MsgBody: msgBody,
			}
			msg, err := absMessage.Marshal()
			if err == nil {
				if cache.SendMessage(playerID, msg) == nil {
					util.LogRecv(absMessage.GetMsgID(), playerID, roomID, buyPaoListRecv, "BuyPaoList")
				}
			}
		}
	}
...
func HandlePrepareGame(s *melody.Session, mjPlayer *cache.MjPlayer, abs *pf.AbsMessage) (err error) {
	...
	// 提醒玩家买跑
	if buyPaoListRecv.Status == def.StatusOK {
		msgBody, err := buyPaoListRecv.Marshal()
		if err == nil {
			absMessage := &pf.AbsMessage{
				MsgID:   int32(pf.BuyPaoList),
			...
			if err == nil {
				...
				for _, deskID := range deskIDs {
					if cache.SendMessage(deskID, msg) == nil {
						util.LogRecv(absMessage.GetMsgID(), deskID, roomID, buyPaoListRecv, "BuyPaoList")
					}
				}
			}
		}
...
func HandleBuyPao(s *melody.Session, mjPlayer *cache.MjPlayer, send *pf.BuyPaoSend, abs *pf.AbsMessage) (err error) {
	playerID := mjPlayer.PlayerID
	roomID := mjPlayer.RoomID
	msgID := abs.MsgID

	buyPaoRecv := &pf.BuyPaoRecv{
		Status: def.StatusOK,
	}
	startRoundRecv := &pf.StartRoundRecv{
		Status: def.StatusFailed,
	...
	if err == nil {
		...
		if err == nil {
			err = s.WriteBinary(recv)
			if err == nil {
				util.LogRecv(msgID, playerID, roomID, buyPaoRecv, "BuyPao")
			}
		}
	}

	// 买跑给其他人发消息
	if buyPaoRecv.Status == def.StatusOK {
		msgBody, err := buyPaoRecv.Marshal()
		if err == nil {
			absMessage := &pf.AbsMessage{
				MsgID:   int32(pf.BuyPao),
				MsgBody: msgBody,
			}
			msg, err := absMessage.Marshal()
			if err == nil {
				for _, otherID := range otherIDs {
					if cache.SendMessage(otherID, msg) == nil {
						util.LogRecv(absMessage.GetMsgID(), otherID, roomID, buyPaoRecv, "BuyPao")
					}
				}
			}
		}
