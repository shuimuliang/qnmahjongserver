...
func (t Tiles) IsHupai(seat *MJSeat, config *MJConfig) (huTiles Tiles, fanshu Fanshu, ok bool) {
	...
	if ok {
		rulerTiles := seat.GetRulerTiles()
		tempTiles := append(rulerTiles, result...)
		fanshu = tempTiles.GetFanshuElse(config)
...
func (t Tiles) IsHupaiWithHun(seat *MJSeat, config *MJConfig, hunCnt int) (huTiles Tiles, fanshu Fanshu, ok bool) {
	// if Hu_7pair_hun(t, int(hunCnt)) {
	// 	fanshu = t.GetFanshu7Pair()
	// 	for i := 0; i < hunCnt; i++ {
	// 		t = append(t, *config.HunTile)
	// 	}
	// 	huTiles = t
	// 	ok = true
	// 	return
	...
	// if Hu_13yao_hun(t, int(hunCnt)) {
	// 	fanshu = t.GetFanshu13yao()
	// 	for i := 0; i < hunCnt; i++ {
	// 		t = append(t, *config.HunTile)
	// 	}
	// 	huTiles = t
	...
	for _, tiles := range GetpossibleTiles(t, hunCnt, config.Config.Daifengpai) {
		tiles.SortTiles()
		if len(tiles)%3 != 2 {
			return
		}
	...
	if ok {
		rulerTiles := seat.GetRulerTiles()
		tempTiles := append(rulerTiles, result...)
		fanshu = tempTiles.GetFanshuElseWithHun(config)
...
func Hu_13yao(tiles Tiles) (ok bool) {
	if len(tiles) == 14 {
		var j = 0
		var falg = false
		for i := 0; i < 14; i++ {
			if tiles[i].Unicode != shiSanYao[j] {
				return
			}
			if !falg {
				if tiles[i+1].Unicode == shiSanYao[j] {
					falg = true
					i++
				}
			}
...
func Hu_13yao_hun(tiles Tiles, hunCnt int) (ok bool) {
	tiles = tiles.DelHunCard()
	tiles.SortTiles()
	length := 14 - hunCnt
	pair := 0
...
func inShiSanYao(tile Tile) (ok bool) {
	for _, temp := range shiSanYao {
		if temp == tile.Unicode {
			ok = true
		}
	}
	return
